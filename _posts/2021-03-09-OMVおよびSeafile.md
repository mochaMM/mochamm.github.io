...
&nbsp;&nbsp;看了其他的方法，要做debian的光盘启动盘，进入repair模式，太复杂，索性重装吧。重装的时候，先用一个新工具不行（也有可能是用前期用diskGenius只做了分区，没有格式化的缘故），<Br/>
&nbsp;&nbsp;因为安装进行到设置网络DHCP的时候，之前openwrt的lan口应该是设置了静态IP，这块就卡死了。后来换了DiskGenius格式化U盘为Fat32之后，再用UltraIso,烧录后，<Br/>
